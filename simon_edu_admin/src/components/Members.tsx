                ...
                filteredUsers.map((user) => {
                  ...
                  return (
                    <tr key={user.id}>
                      ...
                      <td>{user.name || '-'}</td>
                      <td>{user.email}</td>
                      <td style={{ fontFamily: 'var(--font-en)', fontWeight: 'bold' }}>{user.points.toLocaleString()}P</td>
                      <td style={{ fontSize: '0.8rem' }}>{getVerseText(user.currentVerseIndex)}</td>
                      <td style={{ fontFamily: 'var(--font-en)' }}>{user.consecutiveCheckIns}일</td>
                      <td>
              ...
              <div className="mobile-user-card" key={user.id}>
                ...
                <div className="mobile-user-actions">
                  ...
                  <button
                    onClick={() => handleDeleteUser(user)}
                    className="btn-icon-action"
                    style={{ color: 'var(--accent-rose)', width: '38px', height: '38px' }}
                    title="계정 삭제"
                  >
                    <span className="material-icons-round" style={{ fontSize: '1.1rem' }}>delete</span>
                  </button>
          ...
          <div className="glass-panel modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '650px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '1.25rem' }}>
              <div>
                ...
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>가입 상태 / 권한</p>
                <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.2rem' }}>
                  <span className={`badge ${selectedUser.role}`}>{selectedUser.role}</span>
                  <span className={`badge ${selectedUser.status || 'active'}`}>{selectedUser.status || 'active'}</span>
                </div>
              </div>
